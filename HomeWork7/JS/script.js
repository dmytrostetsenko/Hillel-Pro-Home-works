...
const getAverageGrades = () => {
    test.map(student => {
    student.average = student.marks.reduce((prev, current) => prev + current) / student.marks.length;
    });
};

const getWorstStudents = () => {
    getAverageGrades();
    const worstStudents = test.filter(student => student.average < 60);
...
const getMedian = () => {
    test.map(student => {
        ...
        const mid = Math.floor(student.marks.length / 2);
      student.median = student.marks.length % 2 !== 0 ? student.marks[mid] : (student.marks[mid - 1] + student.marks[mid]) / 2;
    });
    console.log(test);
};

const addStudent = (name, specialty, marks) => {
    const newStudent = {
        name: name,
        specialty: specialty,
        marks: marks,
...
const sortStudents = () => {
    getAverageGrades();
    test.sort((a, b) => b.average - a.average);
...
const getBestStudents = () => {
    getAverageGrades();
    const bestStudents = test.sort((a, b) => b.average - a.average).slice(0, 5);
